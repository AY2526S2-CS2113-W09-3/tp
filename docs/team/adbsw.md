...
* Features implemented: 
  * Added the ability to view a list of all inventory items in the chronological order that the items are added, or view a list of all inventory items sorted by a desired property and order specified by the users.
    * Justification: This allows shopowners to view their inventory of items all at once, with the ease of sorting it (e.g. by the quantities of goods) so that they can gain more information about the goods (e.g goods that are low on stock, goods that priced high) and their comparison with other goods at a glance.
...
* Contributions: Updating UML diagrams and sections of the User and Developer guides not specific to a feature (Overall architecture diagram, editing of UML diagrams to follow standard conventions, updating of user stories in the Developer Guide)
